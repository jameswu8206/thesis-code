...
#define ND 1
#define N  20   // Prediction Horizon steps
...
void build_bounds(OSQPFloat **l_ptr, OSQPFloat **u_ptr, const OSQPFloat x_current[NX], const OSQPFloat x_min[NX], const OSQPFloat x_max[NX], const OSQPFloat u_min[NU], const OSQPFloat u_max[NU], OSQPFloat d_est[ND], const OSQPFloat E[NX][ND]){
...
int main(void) {
    ...
    // System Dynamics (Discretized Euler): x_k+1 = Ad*x_k + Bd*u_k +Ed
    // Ad = I +Ac*DT
    // Bd = Bc*DT
    // E for disturbance is known
    //can use matlab c2d function to check actual value
    const OSQPFloat Ad[NX][NX] = {{0.99836847, 0.00996684},
                                {-0.32591565, 0.99283133}};
    const OSQPFloat Bd[NX][NU] = {{0.00277189},
                                {0.55371331}};
    ...
    const OSQPFloat Q_diag[NX] = {50.0, 10.0}; //for fast response
    const OSQPFloat R_diag[NU] = {100.0}; //penalize large input       
    const OSQPFloat P_diag[NX] = {1000.0, 100.0};//for damping fater to desired point
    ...
    OSQPFloat x_current[NX] = {0.02,0.0};//initial point
    ...
    if(opt_time==1){//best timing but jagged behavior
        ...
        settings->alpha = 1.0;
        settings->verbose = 0;
    }if(opt_time==2){// smooth while still better in time due to optimized P cost
        settings->adaptive_rho = 1;//must enable
    ...
    for (int step = 0; step < MAX_STEPS; step++) {
        ...
        // Print all states
        for(int i=0; i<NX; i++) {
            printf(" %6.4f", x_current[i]);
        }
        printf(" |");
        // Print all controls
        for(int i=0; i<NU; i++) {
            printf(" %6.4f", u_applied[i]);
        }
        printf("\n");
        ...
        for (int i = 0; i < NX; i++) {
        x_next[i] = solver->solution->x[NX + NU + i];
        }
        ...
        */
        ...
        int converged = 1;
        for(int i=0; i<NX; i++) {
            if (fabs(x_current[i] - x_target[i]) > 0.0001) {
                converged = 0;
                break;
            }
        }
